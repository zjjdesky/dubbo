 ...
 * 结合dubbo实现的时间轮，实现失败重试的能力；
 * 真正与服务发现组件的交互能力则是放到了 doRegister()/doUnregister()、doSubscribe()/doUnsubscribe() 以及 doNotify() 这五个抽象方法中，
 * 由具体子类实现。这是典型的模板方法模式的应用。
 */
public abstract class FailbackRegistry extends AbstractRegistry {
     ...
     * 然后从传入的 URL 参数中获取重试操作的时间间隔（即retry.period 参数）来初始化 retryPeriod 字段，最后初始化 retryTimer时间轮。
     * @param url
     */
    ...
    private void addFailedRegistered(URL url) {
        ...
        if (oldOne != null) { // 如果存在重试任务，则无需创建，直接返回
            return;
        }
    ...
    private void removeFailedSubscribed(URL url, NotifyListener listener) {
        Holder h = new Holder(url, listener);
        FailedSubscribedTask f = failedSubscribed.remove(h); // 清理FailedSubscribedTask
        if (f != null) {
            f.cancel();
        }
