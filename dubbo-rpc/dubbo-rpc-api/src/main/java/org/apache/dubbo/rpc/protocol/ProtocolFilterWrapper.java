...
public class ProtocolFilterWrapper implements Protocol {
     ...
     * 2. 遍历 Filter 集合，将每个 Filter 实现封装成一个匿名 Invoker，
     *    在这个匿名 Invoker 中，会调用 Filter 的 invoke() 方法执行 Filter 的逻辑，
    ...
    private static <T> Invoker<T> buildInvokerChain(final Invoker<T> invoker, String key, String group) {
        ...
        if (!filters.isEmpty()) {
            // 倒序循环Filter，创建Filter链的Invoker对象
                                ...
                                try {
                                    if (listener != null) {
                                        if (t == null) {
                                            ...
                                            // 当invoke() 方法执行出现异常时，会调用该 Listener 的 onError() 方法进行通知。
